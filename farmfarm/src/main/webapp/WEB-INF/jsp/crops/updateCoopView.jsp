...
.wrap_content {	overflow: hidden; height: 520px; margin: 0 auto; width: 708px;}
...
<c:if test="${loginInfo == null }">
	...
	<div class="wrap_content row">
		...
		<div class="addForm col-xs-6">
			<form action="${pageContext.request.contextPath}/crops/updateCrops.do" style="width:270px; border: 3px solid gray;" method="post">
			<input id="submit" type="submit" class="btn btn-info" value="좌표수정" disabled="disabled"/><br>
				<input id="mbrId" name="mbrId" readonly="readonly" value="${loginInfo.mbrId}"/>
				<input id="itemCode" name="itemCode" readonly="readonly" value="${itemCode}" type="text"style="width:100px;"/><br>
				<input type="text" id="latitude" hidden="hidden" name="latitude" readonly="readonly" placeholder="위도"/><br>
				<input type="text" id="longitude" hidden="hidden" name="longitude" readonly="readonly" placeholder="경도"/><br>
			</form>
			<a id="pointBtn" class="btn btn-danger" onclick="selectPoint()">좌표확정</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
			<a id="reset" href="${pageContext.request.contextPath}/crops/updateCoopView.do?itemCode=${itemCode}" ><img src="<c:url value='/images/makeImage/button/resetBtn.png' />" width="60" height="40" /></a><br>
			<div id="point" style="overflow: scroll;" hidden="hidden">
				<b id='cnt'></b><br>
				<span id='lat'></span><br>
				<span id='lng'></span>
			</div>
	...
	<script>
			 ...
			 if(res.length != 0){
				...
				if(res.length != 0){						// 입력한 정보가 없으면 이부분은 그냥 넘어감
					for(var i=0; i< res.length; i++) {
						...
						area.push({ 'name':res[i].scode,'itemCode':res[i].itemCode ,'path':path ,'strokeColor':res[i].strokeColor });
						displayArea(area[i]);
						drawMarker(area[i]);
					}
				}
		...
		function drawMarker(area){
			var selectCode = "<c:out value='${itemCode}'/>"
			if(area.itemCode == selectCode) {
				/* // 마커가 표시될 위치입니다 
				var markerPosition  = new daum.maps.LatLng(area.path[0].hb, area.path[0].gb); 
				...
				var iwContent = '<div style="padding:5px;">선택한작물</div>', // 인포윈도우에 표출될 내용으로 HTML 문자열이나 document element가 가능합니다
			    iwPosition = new daum.maps.LatLng(area.path[0].hb, area.path[0].gb), //인포윈도우 표시 위치입니다
